...
(function(global, doc, factory) {
	...
	function hasClass(elem, classSingle) {
		return (elem.className && elem.className.match(new RegExp('(\\s|^)' + classSingle + '(\\s|$)'))) ? true :
			false;
	}
	...
	function Stick(param) {
		...
		this.scrollListener = function() {
			...
			if (now - last_time > 500 && (getScrollTop() + window.innerHeight >= document.body.scrollHeight - me
					.load_spacing)) {
	...
	Stick.prototype = {
		buildLayout: function() {
			...
			this.list = [];
			this.last_row = [];
			this.column_num = Math.floor((width + this.column_gap) / (this.column_width_base + this
				.column_gap));
