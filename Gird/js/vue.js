...
(function(global, factory) {
	...
	function renderMixin(Vue) {
		...
		Vue.prototype._m = function renderStatic(
			...
			tree = this._staticTrees[index] = this.$options.staticRenderFns[index].call(this
				._renderProxy);
				...
				if (isUndef(oldStartVnode)) {
					...
					if (isUndef(idxInOld)) { // New element
						...
						if (sameVnode(elmToMove, newStartVnode)) {
							...
							canMove && nodeOps.insertBefore(parentElm, newStartVnode.elm, oldStartVnode
							.elm);
							newStartVnode = newCh[++newStartIdx];
						} else {
		...
		componentUpdated: function componentUpdated(el, binding, vnode) {
			if (vnode.tag === 'select') {
				...
				var needReset = el.multiple ?
					...
					binding.value !== binding.oldValue && hasNoMatchingOption(binding.value, el
						.options);
				if (needReset) {
					trigger(el, 'change');
				}
	...
	function setSelected(el, binding, vm) {
		...
		if (isMultiple && !Array.isArray(value)) {
			"development" !== 'production' && warn(
				...
				"expects an Array value for its binding, but got " + (Object.prototype.toString.call(
					value).slice(8, -1)),
				vm
			);
			return
		...
		while (html) {
			...
			if (!lastTag || !isScriptOrStyle(lastTag)) {
				...
				var rest = html.replace(reStackedTag, function(all, text, endTag) {
					...
					if (stackedTag !== 'script' && stackedTag !== 'style' && stackedTag !==
						'noscript') {
						text = text
							.replace(/<!--([\s\S]*?)-->/g, '$1')
		...
		for (i = 0, l = dirs.length; i < l; i++) {
			...
			if (needRuntime) {
				...
				res += "{name:\"" + (dir.name) + "\",rawName:\"" + (dir.rawName) + "\"" + (dir.value ? (
					...
					dir.arg ? (",arg:\"" + (dir.arg) + "\"") : '') + (dir.modifiers ? (",modifiers:" + (
